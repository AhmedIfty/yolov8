        ...
        "subtypes": {
            ...
            "knife": {
                ...
                "neg": [
                    ...
                    "scissors", "scissor",
                    "pin", "safety pin", "needle", "nail", "thumbtack", "push pin",
                    "fork", "axe"
                ],
            },

            # ====== RAZOR BLADE (maps 'blade' -> here) ======
            "razor": {
                "keywords": ["razor", "razor blade", "safety razor blade", "blade"],
                "pos": [
                    "thin rectangular razor blade with central cutouts",
                    "double edged safety razor blade with notches",
                    "rectangular metal razor blade",
                ],
                "neg": [
                    "knife", "box cutter knife", "paper cutter",
                    "scissors", "fork",
            ...
            "scissors": {
                ...
                "pos": [
                    ...
                    "x-shaped scissors with finger holes",
                    "pair of blades joined at a pivot with looped handles"
                ],
                "neg": [
                    "knife", "razor blade", "box cutter",
            ...
            # ====== PINLIKE (pin + nail collapsed) ======
            "pinlike": {
                "keywords": ["pin", "push pin", "thumbtack", "tack", "nail"],
                "pos": [
                    "thin straight metal shaft",
                    "very slender rod-like object, no handle",
                    "small round head or flat head at one side"
                ],
                "neg": [
                    "needle with an eye", "sewing needle",
                    "knife", "razor blade", "scissors", "fork",
                    "screw", "bolt", "box cutter"
                ],
            },

            # (Optional – keep separate if you ever need exact labels)
            # "needle": {...}  # you can keep it; it will compete as a sibling
            # "safety_pin": {...}
        }
...
def subtype_competition(clip_model, preprocess, tokenizer, image_bgr, boxes_xyxy,
    ...
    neg_t = ((zimg @ zt_neg.T) / TEMP).max(dim=1).values if zt_neg is not None else torch.zeros_like(pos_t)
    ...
    if siblings:
        ...
        for sname in siblings:
            ...
            sn = ((zimg @ zn.T) / TEMP).max(dim=1).values if zn is not None else torch.zeros_like(sp)
...
def clip_simple(clip_model, preprocess, tokenizer, image_bgr, boxes_xyxy,
    ...
    sn = ((zimg @ zneg.T) / TEMP).max(dim=1).values if zneg is not None else torch.zeros_like(sp)
    s  = sp - NEG_BETA*sn
    for j, idx in enumerate(valid_idx):
...
def run_on_image(det, clip_model, preprocess, tokenizer, img_path:Path, query:str,
    ...
    if (route["cfg"] is not None) and (subtype_target is not None) and ("subtypes" in route["cfg"]):
        ...
        # If nothing survives, relax thresholds progressively but STILL compete with siblings
        if n_after_comp == 0:
            # 1) drop the delta, keep a minimal target floor
            relax_keep = (s_t >= max(THR, 0.18)) & ((s_t - s_best) >= 0.00)
            if relax_keep.any():
                keep_mask_extra = relax_keep
                n_after_comp = int(relax_keep.sum())
            else:
                # 2) take top-K by (s_t - s_best) gap (ensures subtype preference),
                #    but cap by fused later anyway
                # --- THIS IS THE CORRECTED NUMPY-ONLY BLOCK ---
                gaps = s_t - s_best                               # NumPy array
                k = int(min(5, gaps.size))
                topk_idx = np.argsort(-gaps)[:k]                  # top-k by gap
                keep_mask_extra = np.zeros_like(gaps, dtype=bool)  # NumPy mask
                keep_mask_extra[topk_idx] = True
                s_clip_n = norm01(s_t)                             # keep as NumPy
                n_after_comp = int(keep_mask_extra.sum())
    else:
